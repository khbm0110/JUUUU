...
const ServiceCard: React.FC<{
  ...
  isInView: boolean;
  lang: string;
}> = ({ service, index, isInView, lang }) => {
  const tilt = useTilt(5);

  const getCardClass = (i: number) => {
    if (i === 0 || i === 3) return 'bento-wide';
    return '';
  };

  const descriptions: Record<string, string> = {
    fr: 'Nous offrons un conseil specialise et une expertise juridique complete dans ce domaine',
    en: 'We provide specialized consulting and comprehensive legal expertise in this area',
    ar: 'نقدم استشارة متخصصة وخبرة قانونية شاملة في هذا المجال',
  };

  const arrowLabel = lang === 'ar' ? 'اعرف المزيد' : lang === 'en' ? 'Learn more' : 'En savoir plus';
  ...
  return (
    <div
      ...
      className={('relative group ' + getCardClass(index) + ' glass-card tilt-card rounded-2xl p-6 md:p-7 flex flex-col justify-between cursor-default transition-all duration-500 ' + (isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'))}
      style={{ transitionDelay: (150 + index * 80) + 'ms', transformStyle: 'preserve-3d' }}
    >
      <div className="relative z-10">
        ...
        {index < 3 && (
          <p className="text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>
            {descriptions[lang] || descriptions.fr}
          </p>
        )}
      </div>

      <div className="relative z-10 mt-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 group-hover:translate-x-1" style={{ color: 'var(--gold)' }}>
        <span className="text-sm font-medium">{arrowLabel}</span>
...
const Services: React.FC = () => {
  ...
  const sectionLabel = state.language === 'ar' ? 'خدماتنا' : state.language === 'en' ? 'Our Services' : 'Nos Services';
  ...
  return (
    <section ref={ref} className="relative py-24 md:py-32 overflow-hidden" style={{ backgroundColor: 'var(--navy-mid)' }}>
      ...
      <div className="deco-ring hidden lg:block" style={{ width: '400px', height: '400px', bottom: '-150px', right: '-100px' }}></div>
      <div className="deco-ring animate-rotate-slow hidden xl:block" style={{ width: '250px', height: '250px', top: '5%', right: '10%', borderColor: 'rgba(201,168,76,0.05)' }}></div>
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16 md:mb-20">
          <div className={('inline-flex items-center gap-2 mb-6 transition-all duration-700 ') + (isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4')}>
            <span className="section-badge">{sectionLabel}</span>
          </div>
        ...
        <div className="bento-grid">
          {services.map((service, index) => (
            <ServiceCard key={index} service={service} index={index} isInView={isInView} lang={state.language} />
          ))}
        </div>
      </div>
